...
from sam2.helpers import DropPath, drop_path
...
class AdaLNSelfAttnNoCond(nn.Module):
    """
    一个去除了 cond_BD / shared_aln / gamma/shift 等条件适应逻辑的自注意力模块。
    相当于标准 Transformer Block (LN -> SelfAttn -> residual -> LN -> FFN -> residual)。
    """
    ...
    def __init__(
        ...
        """
        :param block_idx: 索引，仅用于调试或打印
        :param embed_dim: token 特征的维度 C
        :param num_heads: 注意力头数
        :param mlp_ratio: FFN 的隐层维度与 embed_dim 的比例
        :param drop: dropout 概率
        :param attn_drop: 注意力 dropout 概率
        :param drop_path: drop path 概率
        :param norm_layer: 用于做 LayerNorm 的类
        :param attn_l2_norm: 是否在 SelfAttention 中加 L2 正则 (原先 VAR 里有的选项)
        :param flash_if_available: 是否使用 flash attention 优化
        :param fused_if_available: 是否使用 fused MLP
        """
        super().__init__()
        ...
        # 4) 两个常规 LN
        #    注：原先 AdaLNSelfAttn 用的是 ln_wo_grad + gamma/shift，
        #       这里直接用 elementwise_affine=True 的标准 LN
    ...
    def forward(self, x: torch.Tensor, attn_bias=None) -> torch.Tensor:
        """
        :param x: (B, N, C) 的输入序列
        :param attn_bias: 注意力偏置 (mask、因果遮挡等)，若无可以是 None
        :return: (B, N, C)
        """
        # (1) LN -> SelfAttention -> 残差
        shortcut = x
        x = self.norm1(x)
        x_attn = self.attn(x, attn_bias=attn_bias)  # (B, N, C)
        x = shortcut + self.drop_path(x_attn)

        # (2) LN -> FFN -> 残差
        shortcut = x
        x = self.norm2(x)
        x_ffn = self.ffn(x)  # (B, N, C)
        x = shortcut + self.drop_path(x_ffn)

        return x

    def extra_repr(self) -> str:
        return f'block_idx={self.block_idx}, embed_dim={self.embed_dim}, no_cond=True'
